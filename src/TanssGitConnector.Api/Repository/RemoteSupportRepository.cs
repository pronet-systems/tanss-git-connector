...
public sealed class RemoteSupportRepository : IRemoteSupportRepository
{
    ...
    /// entschieden wird über den genauen Vergleich der Vorgangskennung, und die ist eindeutig.
    ...
    /// Datensatz, weil die Serveruhr abweicht, gilt der Commit als nicht gebucht, und
    ...
    public Task<bool> ExistsAsync(string remoteMaintenanceId, DateTimeOffset around,
                                  CancellationToken ct = default) =>
        // Ein einzelner Bezugszeitpunkt sagt nichts ueber die Dauer. Wer den Vorgang hat,
    ...
    public Task<bool> ExistsAsync(RemoteSupportWrite item, CancellationToken ct = default)
    {
        ...
        DateTimeOffset start = TanssTime.FromUnixSeconds(item.StartTime) ?? now;
        DateTimeOffset end = TanssTime.FromUnixSeconds(item.EndTime) ?? now;

        return ExistsAsync(item.RemoteMaintenanceId, start, end, ct);
    }

    /// <inheritdoc />
    public async Task<bool> ExistsAsync(string remoteMaintenanceId, DateTimeOffset startedAt,
                                        DateTimeOffset endedAt, CancellationToken ct = default)
    {
        ...
        (DateTimeOffset from, DateTimeOffset till) =
            startedAt <= endedAt ? (startedAt, endedAt) : (endedAt, startedAt);
    ...
    /// <remarks>
    /// Der Rumpf selbst gehört nicht in die Meldung: er trägt Kundendaten und Vorgangskennungen.
    /// </remarks>
